...
The jitter/box plot cannot tell us the significant different of the logerror distribution among the months. 


Try the histogram/freqpoly plot. 
...
# It looks better. It seems Zestimate has high accuracy through the whole year.

```


1.3.2 Monthly Change of Logerror by denoting median error rate

```{r,fig.width=10, fig.height=7}

tempB <- ddply(train16, c("month"), summarise, median = median(logerror))
...
ggplot(tempB, aes(x=month, y=median, group=1)) + geom_point(color="brown", size=5) + geom_line(color="brown", size=1.5) + xlab ("Montly Change of the Logerror on Zillow (2016)") + ylab("Logerror") + scale_y_continuous(limits = c(0, 0.015), breaks = seq(0, 0.015, 0.0015)) + geom_smooth(method = "lm", se = FALSE, size = 2, color = "blue") + theme_bw()
...
January, February, September, October, November and December of 2016 = less property data?
March, April, May, June, July and August of 2016 = enough property data?
...
# 1.5 Relationships between the terror and home attributes
...
train16_ts <- subset(train16, select = - c(transactiondate)) # delect the transactiondate that might affect the ts convertation
