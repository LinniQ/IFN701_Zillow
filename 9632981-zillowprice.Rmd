...
```
From the summary, the mean the mix min looks fine...but why the accuracy rate appears so low.
# Calculate prediction accuracy rates (maybe wrong code)
```{r}
pre1_anold <- data.frame(cbind(origin=test1$logerror, pred=pre1_a))
cor_accuracy1_a <- cor(pre1_anold)
print(cor_accuracy1_a) #correlation accuracy10.8%
min_max_accuracy1_a <- mean(apply(pre1_anold, 1, min) / apply(pre1_anold, 1, max)) 
print(min_max_accuracy1_a) #-inf
mape1_a <- mean((abs(pre1_anold$predict.model1_a..test1. - pre1_anold$origin))/pre1_anold$origin)
...
## TEST 3 7:3 (6 variables, delete bathroomcnt & bedroomcnt & roomcnt)
```{r}
