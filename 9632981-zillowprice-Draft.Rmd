...
eg.
    the pattern of the sales (seasonal sales pattern)
...
eg.
    ...
    --- Decision tree doesn't fit this problem, as the return confusionMatrix has no equal x cord and y cord classifers.
    --- Multiple linear regression works okay, but not sure the prediction accuracy
...
nrow(train16[train16$logerror >= 0,]) #50608, overestimation is more than underestimation
nrow(train16[train16$logerror < 0,]) #39667
...
January, February, September, October, November and December of 2016 = less home feature value? Or just caused by the seasonal sales.
March, April, May, June, July and August of 2016 = enough home feature value?


Let's combine prop16 and train16 by parcelid. The result file will tell us the physical features of the sold property in 2016 and their respective error. As well as, the relationships hidden among those factors might be disclosure.
...
#mergeid$logerror <- abs(mergeid$logerror)
...
if it needs to replace the name of months into numeric @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
tempM1 <- subset(tempM,tempM > 58353) #32 fields, including parcelid
print(tempM1)
...
print(tempM2) #19, including parcelid
...
So, what we can learn from these two sections 1.3 and 1.4 is that logerror and transaction frequancy have negative correlation. However, it is hardly to see the correlationship between logerror and other features.
...
plot(tempM2dfn$logerror, col=tempM2dfn$fips)
...
bathE <-select(tempM2dfn, bathroomcnt, logerror)
bathE1 <- ddply(bathE, .(bathroomcnt), "nrow")
bathE2 <- ddply(bathE, ~bathroomcnt, summarise, logerror = mean(logerror))
bathE <- merge(bathE1, bathE2, by = "bathroomcnt")

ggplot(bathE) + geom_line(aes(x= bathroomcnt, y=nrow), color = "purple", size=1.5) + xlab("Bath Room Number and Logerror") + ylab("Frequency")+ scale_y_continuous(limits = c(0, 37000), breaks = seq(0, 37000, 2000)) + scale_x_continuous(limits = c(-0.5, 12), breaks = seq(-0.5, 12, 0.5)) + geom_line(aes(x= bathroomcnt, y=logerror*10^5), color = "brown", size =2) + theme_bw()
...
scatter.smooth(x=tempM2dfn$bedroomcnt/10^3, y=tempM2dfn$logerror, main="bedroomcnt ~ logerror") 

```

```{r}

bedE <-select(tempM2dfn, bedroomcnt, logerror)
bedE1 <- ddply(bedE, .(bedroomcnt), "nrow")
bedE2 <- ddply(bedE, ~bedroomcnt, summarise, logerror = mean(logerror))
bedE <- merge(bedE1, bedE2, by = "bedroomcnt")

ggplot(bedE) + geom_line(aes(x= bedroomcnt, y=nrow), color = "purple", size=1.5) + xlab("Bed Room Number and Logerror") + ylab("Frequency")+ scale_y_continuous(limits = c(0, 36000), breaks = seq(0, 36000, 1000)) + scale_x_continuous(limits = c(-1, 15), breaks = seq(-1, 15,1)) + geom_line(aes(x= bedroomcnt, y=logerror*10^5), color = "brown", size =2) + theme_bw()
...
yearB <-select(mergeid, yearbuilt, logerror)
yearB1 <- ddply(yearB, .(yearbuilt), "nrow")
yearB2 <- ddply(yearB, ~yearbuilt, summarise, logerror = mean(logerror))
yearB <- merge(yearB1, yearB2, by = "yearbuilt")

ggplot(yearB) + geom_line(aes(x= yearbuilt, y=nrow), color = "green", size=1.5) + xlab("Year Built and Logerror") + ylab("Frequency")+ scale_y_continuous(limits = c(0, 2000), breaks = seq(0, 2000, 100)) + scale_x_continuous(limits = c(1880, 2020), breaks = seq(1880, 2020, 10)) + geom_line(aes(x= yearbuilt, y=logerror*10^5), color = "red", size =1) + theme_bw()
...
# finding the regular of logerror for the repeated parcelid (out: the difference between the one is about increasing 0.01 mostly, which is nearthe mean value of the total logerror )

For normal house price prediction, the price normal is increased according to the time series, the increase of the average income in that zone, the profit between the last sold and current sold.
```{r}

a <- subset(mergeid, parcelid== "11842707" | parcelid=="10736972" |parcelid== "10790468"|parcelid=="10796614" |parcelid=="10798910"|parcelid=="10799924"|parcelid=="10815854"|parcelid=="10818076"|parcelid=="10821829"|parcelid=="10798910"|parcelid=="10858080"|parcelid=="10883535"|parcelid=="10911172"|parcelid=="10961914"|parcelid=="11005771"|parcelid=="11061551"|parcelid=="11105038"|parcelid=="11121105"|parcelid=="11121274"|parcelid=="11122560"|parcelid=="11135845")
print(a)
...
# divide the logerror into their targeted classes and name it.
...
## split the data into 90% of training and 10% of testing data
...
# clean the data, keep numeric data only @@@@@@@@@@@@@@@@@@@@@@ if needed , skip first


p1_train <- select_if(p1_train,is.numeric)

p1_test <- select_if(p1_test,is.numeric)
...
#confusionMatrix(predict(tree1, newdata = p1_test),p1_test$logerror)
...
coeffs1 = coefficients(logerror.lm)[1]
print(coeffs1)
coebath = coefficients(logerror.lm)[2]
coebed = coefficients(logerror.lm)[3]
coefips = coefficients(logerror.lm)[4]
coelat = coefficients(logerror.lm)[5]
coelong = coefficients(logerror.lm)[6]
coeprotype = coefficients(logerror.lm)[7]
coeroom = coefficients(logerror.lm)[8]
coetax = coefficients(logerror.lm)[9]
coesqfeet = coefficients(logerror.lm)[10]
coeyear = coefficients(logerror.lm)[11]
...
NO need to care about the varibales, just focus on the train16 data. Use the month and detail date as vaiable to find out if more data and provide a better prediction
```{r}
train16$transactiondate <- as.numeric(train16$transactiondate)

train16$transactiondate <- gsub("[/]", "" , train16$transactiondate, perl=TRUE)

is.numeric(train16$month)

train16$month <- match(train16$month, month.name)

mergeid$transactiondate <- gsub("[/]", "" , mergeid$transactiondate, perl=TRUE)
mergeid$transactiondate <- as.numeric(mergeid$transactiondate)
```

Month by MOnth: Train= the 1st day of the month till the last but the 7 days left
However, if there is no transaction during the last 7 days of the month, we need to explore the unuique date for each month to determine the range of the sample.


```{r}

jan <- subset(train16, month == "1")
feb <- subset(train16, month == "2")
mar <- subset(train16, month == "3")
apr <- subset(train16, month == "4")
may <- subset(train16, month == "5")
jun <- subset(train16, month == "6")
jul <- subset(train16, month == "7")
aug <- subset(train16, month == "8")
sep <- subset(train16, month == "9")
oct <- subset(train16, month == "10")
nov <- subset(train16, month=="11")
dec <- subset(train16, month =="12")
...
# 1.4.4 Continue to narrow down tempM2dfn (seperate the parcelid in three counties: Los Angeles, Orange, Ventura)
...
https://www.ffiec.gov/Census/report.aspx?year=2017&county=037&tract=1032.00&state=06&report=demographic   Middle Level Income
https://www.ffiec.gov/census/report.aspx?year=2016&county=037&tract=8002.04&state=06&report=demographic   Upper level Income

```{r}

summarise(LA, Median=median(LA$logerror), Mean= mean(LA$logerror), Max=max(LA$logerror), Min=min(LA$logerror), Std=sd(LA$logerror)) 
...
tempLA1$month <- ordered(tempLA1$month, levels = c("December","November","October","September", "August", "July","June", "May", "April", "March",   "February","January" ))

ggplot(tempLA1) + geom_bar(aes(x= month, y=transactioncount), fill = "pink", stat = "identity") + xlab("Months") + ylab("Transaction Counts of Properties in LA (2016)")+ scale_y_continuous(limits = c(0, 8000), breaks = seq(0, 8000, 800)) + geom_hline(yintercept = mean(tempLA1$transactioncount),linetype = "dashed", size = 2, color = "brown") + coord_flip() + theme_bw()
```
...
```{r,fig.width=10, fig.height=7}
summarise(OR, Median=median(OR$logerror), Mean= mean(OR$logerror), Max=max(OR$logerror), Min=min(OR$logerror), Std=sd(OR$logerror)) 
```

```{r,fig.width=10, fig.height=7}
tempOR1 <- ddply(OR, .(month), "nrow") 
names(tempOR1)[2] <- c("transactioncount")
tempOR1$month <- ordered(tempOR1$month, levels = c("December","November","October","September", "August", "July","June", "May", "April", "March",   "February","January" ))

ggplot(tempOR1) + geom_bar(aes(x= month, y=transactioncount), fill = "pink", stat = "identity") + xlab("Months") + ylab("Transaction Counts of Properties in OR (2016)")+ scale_y_continuous(limits = c(0, 4000), breaks = seq(0, 4000, 400)) + geom_hline(yintercept = mean(tempOR1$transactioncount),linetype = "dashed", size = 2, color = "brown") + coord_flip() + theme_bw()
```
